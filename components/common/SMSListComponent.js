              ...
              <div className="sms-details">
                {
                	...
                	<div style={{backgroundColor: '#E0FFFF'}}>
                  		{sms.type === 'FIRE' && <span className="glyphicon glyphicon-fire"  onClick={() => {p.selectALocation(sms);}}> {sms.text}  </span>} 
                  		{sms.type === 'AMBULANCE' && <span className="glyphicon glyphicon-bed"  onClick={() => {p.selectALocation(sms);}}> {sms.text}  </span>}
                  		{sms.type !== 'AMBULANCE' && sms.type !== 'FIRE' && <span className="glyphicon glyphicon-alert"  onClick={() => {p.selectALocation(sms);}}> {sms.text}  </span>}
                	</div> 
                }
                {
                	!sms.isOthers &&
