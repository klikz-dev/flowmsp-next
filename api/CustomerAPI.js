...
function getHydrantsChain(
  ...
  showLoader("Loading Hydrant Information", 4, 5);
  AJAXUtil.AJAX({
    ...
    .then((res) => {
      ...
      showLoader("Loaded Hydrants Information", 4, 5);
      showLoader("Loading Pre-Planning Information", 5, 5);
      //   if (typeof successCallback === 'function') {
      //     const message = res.data.data && res.data.data.length > 0 ? null : 'No hydrant found. Right click on the Map to add a hydrant.';
      //     successCallback(message);
      //   }
    })
    .catch(function (error) {
      showLoader("Error Loading Hydrants Information", 4, 5);
...
function getLocationsChain(
  ...
  AJAXUtil.AJAX({
    ...
    .then((res) => {
      ...
      showLoader("Loaded Pre-Planning Information", 5, 5);
      successCallback();
    })
    .catch(function (error) {
      showLoader("Error Loading Pre-Planning Information", 5, 5);
      if (typeof errorCallback === "function") {
...
export function getCustomerList(dispatch) {
  AJAXUtil.AJAX({
    method: "GET",
    url: `${process.env.API_BASE_URL}/api/customer`,
  }).then((res) => {
...
export function createCustomer(form, successCallback, errorCallback) {
  AJAXUtil.AJAX({
    method: "POST",
    data: form,
    url: `${process.env.API_BASE_URL}/api/signup`,
  })
    .then((res) => {
      if (typeof successCallback === "function") {
        successCallback();
...
export function eventSourceMessage(
  ...
  this.source = new EventSource(
    `${process.env.API_BASE_URL}/eventsource?CustomerID=${
